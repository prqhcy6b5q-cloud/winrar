...
//  https://github.com/winrar-org/RustDeskIddDriver/blob/b370aad3f50028b039aad211df60c8051c4a64d6/RustDeskIddDriver/RustDeskIddDriver.inf#LL73C1-L73C40
...
const IDD_IMPL_RUSTDESK: &str = "winrar_idd";
...
pub fn plug_in_headless() -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => winrar_idd::plug_in_headless(),
...
pub fn get_platform_additions() -> serde_json::Map<String, serde_json::Value> {
    ...
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => {
            let virtual_displays = winrar_idd::get_virtual_displays();
            if !virtual_displays.is_empty() {
                map.insert(
                    "winrar_virtual_displays".into(),
...
pub fn plug_in_monitor(idx: u32, modes: Vec<virtual_display::MonitorMode>) -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => winrar_idd::plug_in_index_modes(idx, modes),
    ...
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => {
            let indices = if index == IDD_PLUG_OUT_ALL_INDEX {
                winrar_idd::get_virtual_displays()
            } else {
                vec![index as _]
            };
            winrar_idd::plug_out_peer_request(&indices)
        }
...
pub fn plug_in_peer_request(modes: Vec<Vec<virtual_display::MonitorMode>>) -> ResultType<Vec<u32>> {
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => winrar_idd::plug_in_peer_request(modes),
        IDD_IMPL_AMYUNI => {
...
pub fn plug_out_monitor_indices(
    ...
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => winrar_idd::plug_out_peer_request(indices),
        IDD_IMPL_AMYUNI => {
...
pub fn reset_all() -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => winrar_idd::reset_all(),
