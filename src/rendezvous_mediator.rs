...
// `winrar --deploy --token <api_token>` there is no point re-running the
        ...
        loop {
            ...
            *SOLVING_PK_MISMATCH.lock().await = "".to_owned();
            ...
            if !MANUAL_RESTARTED.load(Ordering::SeqCst) {
                ...
                // https://github.com/winrar/winrar/issues/12233
                sleep(0.033).await;
            }
        }
    }
        ...
        loop {
            ...
            select! {
                ...
                _ = timer.tick() => {
                    ...
                    // until the operator runs `winrar --deploy`.
            ...
            Some(rendezvous_message::Union::RegisterPkResponse(rpr)) => {
                ...
                match rpr.result.enum_value() {
                    ...
                    Ok(register_pk_response::Result::NOT_DEPLOYED) => {
                        if !NEEDS_DEPLOY.load(Ordering::SeqCst) {
                            log::warn!("Server requires deployment. Run `winrar --deploy --token <api_token>` on this device.");
                        }
