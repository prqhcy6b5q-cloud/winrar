...
fn get_update_temp_dir() -> PathBuf {
    ...
    Path::new("/tmp").join(format!(".winrarupdate-{}", euid))
}
...
// remove just one app from all the permissions: tccutil reset All com.winrar.client
...
fn correct_app_name(s: &str) -> String {
    ...
    if let Some(bundleid) = get_bundle_id() {
        s = s.replace("com.winrar.client", &bundleid);
    }
    s = s.replace("winrar", &crate::get_app_name().to_lowercase());
