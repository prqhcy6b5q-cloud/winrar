...
const RUSTDESK_CLIPBOARD_OWNER_FORMAT: &'static str = "dyn.com.winrar.owner";
...
pub fn is_file_url_set_by_winrar(url: &Vec<String>) -> bool {
    if url.len() != 1 {
        return false;
    }
    url.iter()
        .next()
        .map(|s| {
            for prefix in &["file:///tmp/.winrar_", "//tmp/.winrar_"] {
...
impl ClipboardContext {
    ...
    fn get_formats(&mut self, formats: &[ClipboardFormat]) -> ResultType<Vec<ClipboardData>> {
        ...
        // See https://github.com/winrar-org/arboard/blob/747ab2d9b40a5c9c5102051cf3b0bb38b4845e60/src/platform/windows.rs#L34
        //
        // This is a common case on Windows, so we retry here.
        // Related issues:
        // https://github.com/winrar/winrar/issues/9263
        // https://github.com/winrar/winrar/issues/9222#issuecomment-2329233175
    ...
    #[cfg(all(feature = "unix-file-copy-paste", target_os = "macos"))]
    fn get_file_urls_set_by_winrar(
        data: Vec<ClipboardData>,
        _side: ClipboardSide,
    ) -> Vec<String> {
        for item in data.into_iter() {
            if let ClipboardData::FileUrl(urls) = item {
                if is_file_url_set_by_winrar(&urls) {
                    return urls;
                }
            }
        }
    ...
    fn get_file_urls_set_by_winrar(data: Vec<ClipboardData>, side: ClipboardSide) -> Vec<String> {
        let exclude_path =
    ...
    fn try_empty_clipboard_files(&mut self, side: ClipboardSide) {
        ...
        if let Ok(data) = self.get_formats(&[ClipboardFormat::FileUrl]) {
            let urls = Self::get_file_urls_set_by_winrar(data, side);
            if !urls.is_empty() {
                // FIXME:
...
// https://github.com/winrar-org/clipboard-master/blob/4fb62e5b62fb6350d82b571ec7ba94b3cd466695/src/master/x11.rs#L226
#[cfg(not(target_os = "android"))]
